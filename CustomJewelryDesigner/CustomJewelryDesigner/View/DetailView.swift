...
struct DetailView: View {
    ...
    private func gemDisplayName(for gem: GemComponent) -> String {
        let material = gem.color?.capitalized   // "Morganite"
        let shape = gem.cut?.capitalized         // "Oval"

        switch (material, shape) {
        case let (material?, shape?):
            return "\(material) \(shape)"        // "Morganite Oval"
    ...
    private var infoPanel: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(spacing: 16) {
                ...
                infoRow(label: "Gem", value: gemText, swatch: gemColor)
