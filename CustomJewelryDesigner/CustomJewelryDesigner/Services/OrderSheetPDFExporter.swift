...
enum OrderSheetPDFExporter {
    ...
    private static func gemDisplayName(for gem: GemComponent) -> String {
    ...
    private static func gemText(for designFile: DesignFile) -> String {
        let names = designFile.design?.gems.map {
            gemDisplayName(for: $0)
        } ?? []
